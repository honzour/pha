  ...
  TTah1 t1;
   ...
   if (
     DejTah(
       uloha,
       &t1,
		   uloha->pozice.bily ? Nastaveni.CasBileho : Nastaveni.CasCerneho,
       20, NULL, NULL)
       ) {
     Myslim = 0;
...
LRESULT wmlbuttondown(HWND hwnd, int x, int y)
 ...
 if(uloha->KonecPartie)
  ...
  if (x>=0 && x<8*VelPole && y>=0 && y<8*VelPole)
...
LRESULT wmpaint(HWND hwnd)
 {HDC dc;
  ...
  SelectObject(dc,pero);
  SelectObject(dc,brusz);
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
 {
  switch (uMsg){
   case WM_COMMAND:
	...
	{char s[50];
	 ...
	 MessageBox(NULL,s,"Hodnota pozice",MB_OK | MB_TASKMODAL);
	  ...
	  case MVsechnyZnovu:
		  if(Myslim)return 0;
		  while(uloha->prt->p) GlobTahni(1,uloha,0);
		  ZmenPartiar(uloha);
		  MoznaProhodHrace(uloha);
		 PrekresliSachovnici(uloha);
         wmpripravtah(hwnd);
	  ...
	  case MOtoc:
		  ...
		  return 0;
      case MHrajTed:if(Myslim){uloha->StavPropoctu.MamKoncitMysleni=1; return 0;}
   ...
   case WM_LBUTTONDOWN: 
     return wmlbuttondown(hwnd,LOWORD(lParam),HIWORD(lParam));
