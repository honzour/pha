...
#define DlgRozmerX VelPole * 4
#define DlgRozmerY VelPole
...
static LRESULT wmpaint(HWND hwnd)
 {HDC dc,FDC,PDC;
  ...
  for (i=0;i<=3;i++)
   {
    WinTiskniPole(dc,PDC,i&1,0,VelPole*i,0);
    WinTiskniFiguru(dc,FDC,GlobBarva ? 5-i : i-5,VelPole*i+2,2);
   }
  EndPaint(hwnd,&ps);
  DeleteDC(FDC);
  DeleteDC(PDC);
...
LRESULT CALLBACK PromenaWinProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
 {switch(uMsg){
   ...
   case WM_LBUTTONDOWN:
    Kliknul=1;
    GlobVysl=(3-LOWORD(lParam)/VelPole)&3;
