...
static int MojeAtoi(char *s, s32 *i){
/***********************************/
...
/***********************************/
	...
	if(!i || !s || ((s[0]<'0' || s[0]>'9')&&s[0]!='-')) return 0;
	*i=0;j=0;
	if(s[0]=='-'){minus=1;j++;} else minus=0;
	while(1){
		if(j>8 || s[j]>'9' || s[j]<'0')return 0;
		(*i)*=10;
		(*i)+=s[j]-'0';
		j++;
		if(!s[j])break;
...
static int SCioJsouData(int soket){
/************************************************/
...
SCioVypis("Posilam string...");
 ws=(char *)malloc(2*(delka=strlen(s)));
 dws=ws;
 while(*s){
 *dws++ = 0;
 *dws++ = *s++;
 } 
 strcpy(ws,"\000A\000H\000O\000J");
 i=-1-PrjData.typ;
 PosliZpravu(ZPRAVA,4,&i,2,&delka,0);
...
/*******************************************/
 ...
 if(!strcmp(argv[1],"b"))PrjData.bily=1; else
 if(!strcmp(argv[1],"c"))PrjData.bily=0; else
...
static int NactiBuffer(void *buf,int size){
/********************************************************/
/* Pro délku 2 nebo 4 konvertuje poøadí bytù !!         */
/* vrací 1, naèten; 0 nenaèten nebo jen èásteènì */
/********************************************************/
	int i,j;
...
int ZpravaDoBufferu(void *buf /*aspoò na 10 bytù!!*/){
/*************************************************************/
...
#endif
  SCioVypis("Premyslim...");
  if(!DejTah(u,&tah,DelkaMysleni,&(PrjData.PrijmiRemis),&(PrjData.VzdejSe))){
...
*/
 ...
 if(PrjData.PrijmiRemis || PrjData.VzdejSe){
