...
/* tedy mám alespoň dva tahy*/
 ...
 /* HodnotaPozice musím zavolat kvůli nastavení tabulek.
 Navíc mi to pomůže při rozhodování,
 zda se mám vzdát. Nevzdávám se jen tehdy, pokud HodPos >= -věž
 nebo propočet do aspoň jedné hloubky >= -věž nebo mám materiálu za
 dámu. */
  UlozStav(uloha);
...
#endif
        if (pt1->cena <= alfa) {
            /* První tah podtekl alfa. Počítač začíná tusit nestěstí.
...
#ifdef Vypisy
          printf(" AlfaVypAktualizujCenu podteceni");
#endif
