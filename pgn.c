...
#if Typ_Produktu==Win32_Program
static int PrectiDialog(HWND hwnd,char *chyba){
	char s[256];
	...
	if(!IsDlgButtonChecked(hwnd,ID_BilElCh))
	 ...
	 whiteelo=GetDlgItemInt(hwnd, ID_BilEl, NULL, 0);
	 if(!whiteelo){
	...
	if(!IsDlgButtonChecked(hwnd,ID_CerElCh))
	 ...
	 blackelo=GetDlgItemInt(hwnd, ID_CerEl, NULL, 0);
	 if(!blackelo){
		 strcpy(chyba,"Elo èerného musí být kladné celé èíslo !");
		 return 0;
	 }
	}
	round=GetDlgItemInt(hwnd, ID_Kolo, NULL, 0);
	 if(!round){
	...
	rok=GetDlgItemInt(hwnd, ID_Rok, NULL, 1);
	 if(!rok){
		 strcpy(chyba,"Rok musí být nenulové celé èíslo !");
		 return 0;
	 }
	mesic=GetDlgItemInt(hwnd, ID_Mesic, NULL, 0);
	 if(mesic<1 || mesic>12){
		 strcpy(chyba,"Mìsic musí být celé èíslo z <1, 12> !");
		 return 0;
	 }
	den=GetDlgItemInt(hwnd, ID_Den, NULL, 0);
	 if(den<1 || den>31){
...
int CALLBACK PGNDlgWinProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
 {char s[100];
 switch(uMsg){
   case WM_INITDIALOG:
	   ...
	   SetDlgItemInt(hwnd,ID_Kolo,GlobInfo->round,0);
	   SetDlgItemText(hwnd,ID_Komentator,GlobInfo->annotator);
	   SetDlgItemInt(hwnd,ID_BilEl,GlobInfo->whiteelo,0);
	   SetDlgItemInt(hwnd,ID_CerEl,GlobInfo->blackelo,0);
	   SetDlgItemInt(hwnd,ID_Den,GlobInfo->den,0);
	   SetDlgItemInt(hwnd,ID_Mesic,GlobInfo->mesic,0);
	   SetDlgItemInt(hwnd,ID_Rok,GlobInfo->rok,0);
...
int DialogPGN(HWND otec, TPGNInfo *p){
	GlobOK=0;
	GlobInfo=p;
...
int UlozDoPGN(char *jmeno,TUloha *u,TPGNInfo *pgn){
	FILE *f;
	char s[10];
