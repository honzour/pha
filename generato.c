 ...
 for(j=(o1);j<=(o2);j++)\
  {\
   ...
   for((q)=p+ofsety[j],(k)=i+ofsety[j];*(q)<=0;(q)+=ofsety[j],k+=ofsety[j],xo+=ofsetyX[j],yo+=ofsetyY[j])\
    { if(*q<0) {if(q==a)ZaradBileBrani((pzas), i, k, q, (ktera)) break;}\
...
{\
 for(j=(o1);j<=(o2);j++)\
   ...
   *(q)>=0 && *(q)<7;(q)+=ofsety[j],k+=ofsety[j],xo+=ofsetyX[j],yo+=ofsetyY[j])\
    { if(*q>0) {if(q==a)ZaradCerneBrani((pzas),i,k,q, (ktera)) break;}\
      if(MEZI(xo,yo))ZaradTah(pzas,i,k);\
    }\
  }\
}
...
void NalezTahy(TUloha *u) {
  ...
  v zasobniku*/
  pzas = u->zasobnik.tahy + u->zasobnik.hranice[u->zasobnik.pos];
  if (u->pozice.bily) {
  ...
  case 2: /* kun*/
   for (j = 8; j <= 15; j++) 
     if ((*(p+ofsety[j])) <= 0) ZaradBileBrani(pzas,i,i+ofsety[j],(p+ofsety[j]), 2);
   break;
  case 3: /* strelec*/
   DlouhaBilaFigura(pzas,4,7,q,k, 3)
  break;
  case 4: /* vez*/
   DlouhaBilaFigura(pzas,0,3,q,k, 4)
  break;
  case 5: /* dama*/
   DlouhaBilaFigura(pzas,0,7,q,k, 5)
  break;
  case 6: /* kral*/
    *p = 0;
    for (j = 0; j <= 7; j++) {
      const s8 *pkam = p + ofsety[j];
      
      if (*pkam <= 0 && !Ohrozeno(pkam, 0))
        ZaradBileBrani(pzas, i, i + ofsety[j], pkam, 6);
    }
    *p = 6;
  ...
  case -2: /* kun*/
   ...
   break;
  case -3: /* strelec*/
   DlouhaCernaFigura(pzas,4,7,q,k, -3)
  break;
  case -4: /* vez*/
   DlouhaCernaFigura(pzas,0,3,q,k, -4)
  break;
  case -5: /* dama*/
   DlouhaCernaFigura(pzas,0,7,q,k, -5)
  break;
  case -6: /* kral*/
    *p = 0;
    for (j=0;j<=7;j++) {
      ...
      if (*pkam >= 0 && *pkam < 7 && !Ohrozeno(pkam, 1))
        ZaradCerneBrani(pzas, i, i + ofsety[j], pkam, -6)
    }
    *p = -6;
...
{int j,k,i;
 ...
 if (u->pozice.bily)
  ...
  for (i=a1,p=&(u->pozice.sch[a1]); i<=h8; i++,p++)
   ...
   break;
  case 2: /* kun*/
   for (j=8;j<=15;j++) if ((*(p+ofsety[j]))<0) ZaradBileBrani(pzas,i,i+ofsety[j],(p+ofsety[j]), 2);
   break;
  case 3: /* strelec*/
   DlouhaBilaFiguraBrani(pzas, 4, 7, 3)
  break;
  case 4: /* vez*/
   DlouhaBilaFiguraBrani(pzas,0,3, 4)
  break;
  case 5: /* dama*/
   DlouhaBilaFiguraBrani(pzas,0,7, 5)
  break;
  case 6: /* kral*/
   for (j=0;j<=7;j++) if ((*(p+ofsety[j]))<0) ZaradBileBrani(pzas,i,i+ofsety[j],(p+ofsety[j]), 6);
   break;
  }/* od switch*/
 } /* od for cyklu*/
 } /* od hraje bily*/
  ...
  case -2: /* kun*/
   ...
   break;
  case -3: /* strelec*/
   DlouhaCernaFiguraBrani(pzas,4,7, -3)
  break;
  case -4: /* vez*/
   DlouhaCernaFiguraBrani(pzas,0,3, -4)
  break;
  case -5: /* dama*/
   DlouhaCernaFiguraBrani(pzas,0,7, -5)
  break;
  case -6: /* kral*/
   for (j=0;j<=7;j++) if ((*(p+ofsety[j]))>0 && (*(p+ofsety[j]))<7)
        ZaradCerneBrani(pzas,i,i+ofsety[j],(p+ofsety[j]), -6)
  break;
  }/* od switch*/
 } /* od for cyklu*/
 } /* od hraje bily*/
...
#define ZKUS(o,f) {if(*(a+(o))==f&&!VeVazbe(u,pa+(o)))\
ZaradCerneBrani(*pzas,pa+(o),pa,a, (f))}
...
/*********************************************************/
 ...
 for (i=a1,p=&(u->pozice.sch[a1]); i<=h8; i++,p++)
   ...
   break;
  case 2: /* kun*/
   for (j=8;j<=15;j++) if ((*(p+ofsety[j])) <= 0 && MEZI(ofsetyX[j],ofsetyY[j]))
        ZaradBileBrani(*pzas,i,i+ofsety[j],(p+ofsety[j]), 2);
   break;
  case 3: /* strelec*/
   DlSDlouhaBilaFigura(*pzas,4,7,q,k, 3)
  break;
  case 4: /* vez*/
   DlSDlouhaBilaFigura(*pzas,0,3,q,k, 4)
  break;
  case 5: /* dama*/
   DlSDlouhaBilaFigura(*pzas,0,7,q,k, 5)
  break;
  }/* od switch*/
 } /* od for cyklu*/
}
...
/*********************************/
 ...
 for (i=a1,p=u->pozice.sch+a1; i<=h8; i++,p++)
  ...
  break;
  case -4: /* vez*/
   DlSDlouhaCernaFigura(*pzas,0,3,q,k, -4)
  break;
  case -5: /* dama*/
   DlSDlouhaCernaFigura(*pzas,0,7,q,k, -5)
  break;
  }/* od switch*/
 } /* od for cyklu*/
}
...
/**********************************************************/
  ...
  *a=(s8)0;
  for(i=0;i<8;i++)
   ...
   kam=odkud+ofsety[i];
   ZaradBileBrani(pzas,odkud,kam,(a+ofsety[i]), 6)
  }
  *a=(s8)6;
 }
 else /*cerny*/{
  ...
  *a=(s8)0;
  for(i=0;i<8;i++)
   ...
   odkud=a-u->pozice.sch;
   kam=odkud+ofsety[i];
   ZaradCerneBrani(pzas,odkud,kam,(a+ofsety[i]), -6)
  }
  *a=(s8)-6;
  }
