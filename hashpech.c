...
void NastavPesce(TUloha *u, THashPech *ph) {
  ...
  /* for cyklus pres sloupce*/
  for(i = 0; i <= 7; i++) {
    if (u->material.BPSloupec[i] > 1)
      ph->cena += ((u->material.BPSloupec[i] - 1) * u->AlgCfg.AlgKoef.DvojPesec);
    if (u->material.CPSloupec[i] > 1)
      ph->cena -= ((u->material.CPSloupec[i]-1) * u->AlgCfg.AlgKoef.DvojPesec);
